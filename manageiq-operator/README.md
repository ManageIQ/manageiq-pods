...
+ #### Option 2: Run your own custom ManageIQ image inside the Cluster
  ...
  1 - Build your operator image:
    ...
    ```bash
    $ operator-sdk build docker.io/<your_username_or_organization>/manageiq-operator:latest-najdorf
    ```

  2 - Push your new custom image to the registry:

    ```bash
    $ docker push docker.io/<your_username_or_organization>/manageiq-operator:latest-najdorf
    ```

  3 - Update the operator deployment yaml file with your custom image:

    ```bash
    $ sed -i 's|docker.io/manageiq/manageiq-operator:latest-najdorf|docker.io/<your_username_or_organization>/manageiq-operator:latest-najdorf|g' deploy/operator.yaml
    ```
...
See [official documentation](https://www.manageiq.org/docs/reference/latest/installing_on_kubernetes/index.html#external-postgresql)
