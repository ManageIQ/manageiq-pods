...
+ #### Option 2: Run your own custom ManageIQ image inside the Cluster
  ...
  2 - Update the operator deployment yaml file with your custom image:
    ...
    ```bash
    $ sed -i 's|docker.io/manageiq/manageiq-operator:latest-tal|docker.io/<your_username_or_organization>/manageiq-operator:latest-tal|g' config/manager/manager.yaml
    ```
