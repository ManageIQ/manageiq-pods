...
import (
	...
	"github.com/ManageIQ/manageiq-pods/manageiq-operator/test/utils"
)

// constant parts of the file
const namespace = "manageiq-operator-system"

var _ = Describe("manageiq", Ordered, func() {
	BeforeAll(func() {
		// The prometheus and the certmanager are installed in this test
		// because the manageiq sample has this option enable and
	...
	Context("manageiq Operator", func() {
		It("should run successfully", func() {
			...
			var operatorImage = "example.com/manageiq-operator:v0.0.1"
			...
			By("creating an instance of the manageiq Operand(CR)")
			EventuallyWithOffset(1, func() error {
				cmd = exec.Command("kubectl", "apply", "-f", filepath.Join(projectDir,
					"config/samples/cache_v1alpha1_manageiq.yaml"), "-n", namespace)
				_, err = utils.Run(cmd)
				return err
			}, time.Minute, time.Second).Should(Succeed())

			By("validating that pod(s) status.phase=Running")
			getmanageiqPodStatus := func() error {
				cmd = exec.Command("kubectl", "get",
					"pods", "-l", "app.kubernetes.io/name=manageiq-operator",
				...
				if !strings.Contains(string(status), "\"phase\":\"Running\"") {
					return fmt.Errorf("manageiq pod in %s status", status)
				}
				return nil
			}
			EventuallyWithOffset(1, getmanageiqPodStatus, time.Minute, time.Second).Should(Succeed())

			By("validating that the status of the custom resource created is updated or not")
			getStatus := func() error {
				cmd = exec.Command("kubectl", "get", "manageiq",
					"manageiq-sample", "-o", "jsonpath={.status.conditions}",
					"-n", namespace,
				)
				status, err := utils.Run(cmd)
				fmt.Println(string(status))
